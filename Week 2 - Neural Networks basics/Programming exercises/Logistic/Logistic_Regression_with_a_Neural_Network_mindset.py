...
"""
        Find the value for:
            ...
            For instance, you can access m_train by writting train_Set_x_orig.shape[0].
"""
...
"""
        ...
        A trick wwhen you want to flatten a matix X of shape (a, b, c, d) to a matrix X_flatten of shape (b * c * d, a)
        is to use:
...
def initialize_with_zeros(dim):
    """
        ...
        b -- initializzed scalar (corrsponds to the bias) of type float
    """
...
# function that computes the cost function and its gradient.
...
# For a parameter theta, the updata rule is theta = theta - alpha * d_theta,
...
def optimize(w, b, X, Y, num_iterations=100, learning_rate=0.009, print_cost=False):
    ...
    for i in range(num_iterations):
        ...
        # retrieves deriatives from grads
        dw = grads["dw"]
        db = grads["db"]
...
# Predict
"""
    The previous function will output the learned w and b. We are able to use w and  to predict
    the labels for a dataset X. Implement the predict() function. There are two steps to computing
    predictions:
        1. Calculate Y_hat = a = sigmoid(z)
        2. Convert the entries of a into 0 (if activation <= 0.5) or 1 (if activation > 0.5) stores the
        predictions in a vector Y_prediction. If you wish, you can use an if/else statement
        in a for loop (though there is also a way to vectorize this).
"""
...
and has high enough capacity to fit the training data. Test accuracy is 70%. It is actually not bad
for this simple model, given the small dataset we used and that logistic regression is a linear
classifier. But no worries, you'll build an even better classifier next week!
